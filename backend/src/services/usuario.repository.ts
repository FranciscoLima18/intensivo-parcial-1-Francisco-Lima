import { LocalidadUsuario } from "../types/schemas/localidad.js";
import { Usuario } from "../types/schemas/usuario.js";
...
class UsuarioRepository extends BaseRepository<Usuario> {
  ...
  async create(data: Usuario): Promise<Usuario> {
    const consulta =
      "INSERT INTO usuarios (nombre, roles, password_hash) VALUES ($1, $2, crypt('contraseña', gen_salt('bf'))) RETURNING *";
  ...
  async auth(nombre: string, password: string): Promise<Usuario> {
    ...
    if (res.rowCount === 0)
      //Si no existe usuario con esa pass
